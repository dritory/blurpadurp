...
| Worth watching asserted things about live threads with no sources — the one body section that asked for trust without offering a link, because the prompt read its ~15–25-word ceiling as excluding citations (the ceiling is about prose; a citation cluster isn't prose) | Composer v0.12 cites the section, capped at two domains. No input change was needed: `source_url` + `additional_source_urls` were always rendered | `docs/composer-prompt.md` |
